...
#load gamma ray bkg
GammaSigRaw = np.loadtxt('../../MW_GR/GRAhlers/Combined/GFlux_ma'+mass+'_g'+gag+'.dat')
GammaBKGRaw = np.loadtxt('../../MW_GR/GRAhlers/Combined/GFlux_ma'+mass+'_g0.dat')
ALPSigRaw = np.loadtxt('../../GammaALPFluxes/SYICFlux/Mangled/GFlux_B'+mag+'_ma'+mass+'_g'+gag+'.dat')
...
for ii in range(0,blPoints):#rangeL
    ...
    for jj in range(0,len(binG)):
        ...
        chinum_test=nALP[jj,1]
        chiden_test = nP[jj,1]*10**(missID(enL-3))
        ...
        numTest.append([enL,GammaBKGRaw[ii*lenArr,0],GammaBKGRaw[ii*lenArr,1],chinum_test])
        denTest.append([enL,GammaBKGRaw[ii*lenArr,0],GammaBKGRaw[ii*lenArr,1],chiden_test])
...
numTest=np.asarray(sorted(numTest, key=lambda tup: (tup[0],tup[1],tup[2])))
denTest=np.asarray(sorted(denTest, key=lambda tup: (tup[0],tup[1],tup[2])))
...
np.savetxt('DataInt/PNum_B'+str(mag)+'_ma'+str(mass)+'_g'+str(gag)+'.txt',numTest)
np.savetxt('DataInt/PDen_B'+str(mag)+'_ma'+str(mass)+'_g'+str(gag)+'.txt',denTest)
"""
numData=np.loadtxt('DataInt/FullNum_B'+str(mag)+'_ma'+str(mass)+'_g'+str(gag)+'.txt')
denData=np.loadtxt('DataInt/FullDen_B'+str(mag)+'_ma'+str(mass)+'_g'+str(gag)+'.txt')
numTest=np.loadtxt('DataInt/PNum_B'+str(mag)+'_ma'+str(mass)+'_g'+str(gag)+'.txt')
denTest=np.loadtxt('DataInt/PDen_B'+str(mag)+'_ma'+str(mass)+'_g'+str(gag)+'.txt')
...
toSave=[[float(mag),float(mass),float(gag),chiSq]]
np.savetxt('DataRes/resP_B'+mag+'_ma'+mass+'_g'+gag+'.dat',toSave, fmt='%.4e')
np.savetxt('DataGrid/gridP_B'+mag+'_ma'+mass+'_g'+gag+'.dat',grid, fmt='%.4e')
